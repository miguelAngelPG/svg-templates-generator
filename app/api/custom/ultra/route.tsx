...
async function getFonts() {
  const [regular, bold] = await Promise.all([
    fetch('https://fonts.gstatic.com/s/notosans/v30/o-0IIpQlx3QUlC5A4PNb4g.ttf').then((res) => res.arrayBuffer()),
    fetch('https://fonts.gstatic.com/s/notosans/v30/o-0NIpQlx3QUlC5A4PNjXhFVadyB1Wk.ttf').then((res) => res.arrayBuffer()),
  ]);
      ...
      card: (
        <div style={{ width: '100%', height: '100%', display: 'flex', backgroundColor: t.bg, position: 'relative' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', margin: '30px', padding: '40px', background: t.glass, border: `1px solid ${t.border}`, borderRadius: '20px', position: 'relative' }}>
            ...
            <div style={{ fontSize: '32px', fontWeight: 'bold', color: t.text, marginBottom: '15px' }}>{title}</div>
            <div style={{ fontSize: '18px', color: t.secondary, lineHeight: 1.6 }}>{content}</div>
            <div style={{ width: '80px', height: '3px', background: t.accent, marginTop: '25px', borderRadius: '2px' }} />
          </div>
        </div>
      ),
      ...
      quote: (
        <div style={{ width: '100%', height: '100%', display: 'flex', backgroundColor: t.bg, alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', padding: '50px', maxWidth: '90%', position: 'relative' }}>
            ...
            <div style={{ fontSize: '64px', color: t.accent, opacity: 0.3, marginBottom: '20px', marginLeft: '20px' }}>"</div>
            <div style={{ fontSize: '28px', fontStyle: 'italic', color: t.text, lineHeight: 1.5, marginLeft: '20px' }}>{content}</div>
            <div style={{ fontSize: '16px', color: t.secondary, marginTop: '20px', marginLeft: '20px' }}>— {title}</div>
          </div>
        </div>
      ),
      ...
      stat: (
        <div style={{ width: '100%', height: '100%', display: 'flex', backgroundColor: t.bg, alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', position: 'relative' }}>
            <div style={{ fontSize: '56px', marginBottom: '25px' }}>{icon}</div>
            <div style={{ fontSize: '96px', fontWeight: 'bold', color: t.text, marginBottom: '10px' }}>{value}</div>
            <div style={{ fontSize: '24px', color: t.accent, fontWeight: '600', letterSpacing: '2px', textTransform: 'uppercase' }}>{label}</div>
            {content && <div style={{ fontSize: '16px', color: t.secondary, marginTop: '15px' }}>{content}</div>}
          </div>
        </div>
      ),
        ...
        <div style={{ width: '100%', height: '100%', display: 'flex', backgroundColor: t.bg, alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px', padding: '25px 40px', background: t.glass, border: `2px solid ${t.border}`, borderRadius: '50px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '5px' }}>
              <div style={{ fontSize: '28px', fontWeight: 'bold', color: t.text }}>{title}</div>
              <div style={{ fontSize: '16px', color: t.accent }}>{content}</div>
            </div>
          </div>
        </div>
      ),
